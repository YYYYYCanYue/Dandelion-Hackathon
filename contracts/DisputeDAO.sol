...
contract DisputeDAO {
    ...
    // 恶意任务仲裁事件
    event MaliciousTaskResolved(uint256 indexed disputeId, address[] jurors);
    ...
    // 仲裁员奖励常量（单位：wei）
    uint256 public constant JUROR_REWARD = 0.003 ether;

    // 恶意任务仲裁，奖励仲裁员并归集资金
    function arbitrateMaliciousTask(uint256 disputeId) external onlyTaskFactory {
        address[] memory disputeJurors_ = disputeJurors[disputeId];
        require(disputeJurors_.length == jurorsPerDispute, "Dispute not started or juror count mismatch");
        // 通知TaskFactory处理恶意任务，奖励仲裁员
        (bool success, ) = taskFactory.call(
            abi.encodeWithSignature("resolveMaliciousTask(uint256,address[],uint256)", disputeId, disputeJurors_, JUROR_REWARD)
        );
        require(success, "Malicious resolve failed");
        emit MaliciousTaskResolved(disputeId, disputeJurors_);
    }
