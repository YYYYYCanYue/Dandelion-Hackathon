        ...
        <div class="bg-white rounded-lg shadow-lg p-6">
          ...
          <div class="space-y-4">
            ...
            <div class="flex space-x-3">
              ...
              >
                <span v-if="initializingData">初始化中...</span>
                <span v-else>初始化示例数据</span>
              </button>
              
              <button
                @click="loadExistingData"
                :disabled="!getDefaultDataHash()"
                class="flex-1 bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                重新加载数据
              </button>
            </div>
            ...
            <div class="text-sm text-gray-600 bg-gray-50 rounded-lg p-3">
              ...
              <ul class="space-y-1 text-xs">
                <li>• 初始化示例数据将创建5个示例任务、竞标记录和争议数据</li>
                <li>• 数据将上传到本地IPFS节点并返回哈希值</li>
                <li>• 数据哈希会自动保存到localStorage中</li>
                <li>• 重新加载数据将从已保存的哈希中恢复数据</li>
              </ul>
            </div>
          </div>
        </div>
...
import { initializeData, loadDataFromIPFS, getDefaultDataHash } from '@/utils/init-data'
...
export default {
  ...
  setup() {
    ...
    // 添加数据初始化方法
    const initializeIPFSData = async () => {
      initializingData.value = true
      try {
        const result = await initializeData()
        
        if (result.success) {
          dataHash.value = result.hash
          dataInitialized.value = true
          
          // 更新dataStore的数据索引哈希
          dataStore.dataIndexHash = result.hash
          
          // 初始化dataStore的数据
          await dataStore.initializeData()
          
          console.log('✅ IPFS数据初始化成功')
          console.log('数据哈希:', result.hash)
          
        } else {
          console.error('❌ IPFS数据初始化失败:', result.error)
        }
      } catch (error) {
    ...
    const loadExistingData = async () => {
      const existingHash = getDefaultDataHash()
      if (existingHash) {
        dataHash.value = existingHash
        dataInitialized.value = true
        
        // 更新dataStore
        dataStore.dataIndexHash = existingHash
        await dataStore.initializeData()
        
        console.log('✅ 已加载现有数据，哈希:', existingHash)
      }
    }

    onMounted(async () => {
      await testWeb3Import()
      await testIPFSConnection()
      await testMetaMaskAvailability()
      await loadExistingData() // 检查是否有现有数据
    })
